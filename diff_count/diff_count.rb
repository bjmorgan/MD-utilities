...
while ( tline = file.gets )
  ...
  line.shift
  line.each_with_index { |value, index| ions[index].new_site.id = value.to_i }
  ions.each do |ion|
...
printf "%.5f => %.5f pct\tOh diffusion events\n" % [ndiff_oct.to_f / step.to_f, 100*ndiff_oct.to_f/nFrenkel_oct.to_f ]
printf "%.5f => %.5f pct\tTd diffusion events\n" % [ndiff_tet.to_f / step.to_f , (100*ndiff_tet.to_f/nFrenkel_tet.to_f) ]